...
def cleandf(df,Eslab,Vibenefile,rem_avail=False):
	...
	print(df3)
	if (rem_avail):
		drpind = df3[ ~df3["Remarks"].isin(rem) ].index	
...
def plotAvgAdsene(dfEads,df,input_file,system):
	...
	for i, txt in enumerate(rem):
		...
		txtsplt  = txt.split("#")
		txt = "".join(txtsplt)
		ax1.annotate(txt,(xmin[i],ymin[i]),fontsize=8,\
		textcoords="offset points",\
		xytext=xytxt,ha='center',va='bottom',\
		arrowprops=dict(arrowstyle='->', connectionstyle='arc3,rad=0.2'))
		ax2.annotate(txt,(xmin2[i],ymin2[i]),fontsize=8,\
		textcoords="offset points",\
		xytext=xytxt,ha='center',va='bottom',\
		arrowprops=dict(arrowstyle='->', connectionstyle='arc3,rad=0.2'))
...
def plotdGibbs(df,input_file,system):
	...
	for i, txt in enumerate(rem):
		...
		txtsplt  = txt.split("#")
		txt = "".join(txtsplt)
		plt.annotate(txt,(xmin[i],ymin[i]),fontsize=8,\
		textcoords="offset points",\
		xytext=xytxt,ha='center',va='bottom',\
		arrowprops=dict(arrowstyle='->', connectionstyle='arc3,rad=0.2'))
...
def plotdAdsEne(df,input_file,system):
	...
	df2["diffGibbs"] = df2["diffGibbs"]  - eU
	xmin,ymin,rem = df2["nH"], df2["diffGibbs"],df2["Remarks(n,n-1)"]
	for i, txt in enumerate(rem):
		xytxt=(20,-20)
		txtsplt  = txt.split("#")
